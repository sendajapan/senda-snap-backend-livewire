    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-start justify-between gap-3">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    <h3 class="text-sm font-bold text-gray-900 dark:text-white whitespace-nowrap truncate">{{ $task->title }}</h3>
                ...
                @if($task->description)
                    <p class="mt-2 text-xstext-gray-600 dark:text-gray-400 line-clamp-2">{{ $task->description }}</p>
                @endif
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            @if($showWorkDate && $task->work_date)
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-900 dark:text-white whitespace-nowrap">
            ...
            @if($task->assignedUsers && $task->assignedUsers->count() > 0)
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        <span class="truncate text-xs text-gray-900 dark:text-white whitespace-nowrap">
            ...
            @else
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap">{{ __('Unassigned') }}</span>
                </div>
            @endif
                        ...
                        <div class="relative h-6 w-6 flex-shrink-0">
                            ...
                            @else
                                <div class="flex h-6 w-6 items-center justify-center rounded-lg bg-emerald-400/20 ring-2 ring-emerald-300 dark:ring-emerald-800">
                                    <span class="text-xs font-bold text-emerald-900 dark:text-emerald-200">{{ $task->creator->initials() }}</span>
                                </div>
                            @endif
                        </div>
                        <span class="truncate text-xs text-gray-900 dark:text-white whitespace-nowrap">{{ $task->creator->name }}</span>
                    </div>
                </div>
            @endif
        </div>
        ...
        <div class="flex items-center justify-end gap-2 border-t border-gray-200/50 pt-3 dark:border-gray-700/50">
            ...
            <button @click="openPreview({{ $task->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-emerald-700/60 bg-emerald-500/10 p-2 transition-shadow duration-200 hover:border-emerald-700 hover:bg-emerald-500/20 hover:shadow-lg hover:shadow-emerald-700/30" title="{{ __('View Task') }}">
                ...
                <span class="ml-1 text-xs font-semibold text-emerald-700 whitespace-nowrap">{{ __('View') }}</span>
            </button>
            ...
            <button @click="openModal({{ $task->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-cyan-700/60 bg-cyan-500/10 p-2 transition-shadow duration-200 hover:border-cyan-700 hover:bg-cyan-500/20 hover:shadow-lg hover:shadow-cyan-700/30" title="{{ __('Edit Task') }}">
                ...
                <span class="ml-1 text-xs font-semibold text-cyan-700 whitespace-nowrap">{{ __('Edit') }}</span>
            </button>
