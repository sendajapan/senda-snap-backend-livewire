@props([
    ...
    'description',
    'icon',
    'variant' => 'blue', // blue, cyan, emerald, violet, amber, gray
])
...
<div class="page-card group relative rounded-2xl border {{ $classes['border'] }} {{ $classes['bg'] }} {{ $classes['shadow'] }} p-4 ring-1 ring-white/20 transition-shadow duration-300 dark:ring-white/10 {{ $classes['hoverShadow'] }} {{ $classes['hoverBorder'] }}">
    ...
    <div class="relative flex items-center justify-between gap-4 z-10">
        <div class="flex items-center gap-4 min-w-0">
            @if(isset($icon))
                <div class="flex h-icon-md w-icon-md flex-shrink-0 items-center justify-center rounded-3xl {{ $classes['iconBg'] }} shadow-sm text-lg [&_svg]:size-[1em]">
                    {!! $icon !!}
                </div>
            @endif
            <div class="min-w-0">
                <h1 class="text-base md:text-lg lg:text-xl font-bold text-gray-900 dark:text-white truncate">{{ $title }}</h1>
                <p class="text-sm md:text-base text-gray-600 dark:text-gray-400 line-clamp-2 md:line-clamp-none">{{ $description }}</p>
            </div>
        </div>
        @if(isset($actions))
            <div class="flex-shrink-0 [&_button]:text-sm md:[&_button]:text-base">
                {{ $actions }}
            </div>
        @endif
    </div>
