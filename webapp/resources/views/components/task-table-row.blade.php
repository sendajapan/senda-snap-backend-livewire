...
<tr class="group hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10">
    ...
    @if($showTimeFirst)
        <td class="whitespace-nowrap px-3 py-3">
            @if($task->work_time)
                <div class="inline-flex items-center gap-1 rounded-sm bg-accent/90 dark:bg-emerald-600/90 px-1 py-1 shadow-sm">
                    <svg class="h-4 w-4 md:h-5 md:w-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span class="text-xs font-semibold text-white">{{ Carbon::parse($task->work_time)->format('h:i A') }}</span>
                </div>
            @else
                <div class="inline-flex items-center gap-1 rounded-sm bg-zinc-400 dark:bg-zinc-600 px-1 py-1 shadow-sm">
                    <svg class="h-4 w-4 md:h-5 md:w-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span class="text-xs font-semibold text-white">{{ __('No Time') }}</span>
                </div>
            @endif
        </td>
    @endif

    <td class="w-1/4 whitespace-normal px-3 py-3">
        <div class="flex flex-col">
            <div class="flex items-center gap-1 flex-wrap">
                <span class="text-sm font-bold text-gray-900 dark:text-white break-words">{{ $task->title }}</span>
                @if($task->attachments && $task->attachments->count() > 0)
                    <div class="flex items-center gap-1 rounded-md bg-emerald-100 px-1 py-0.5 dark:bg-emerald-900/30 flex-shrink-0" title="{{ $task->attachments->count() }} {{ __('attachment(s)') }}">
    ...
    <td class="whitespace-nowrap px-3 py-3 hidden lg:table-cell">
    ...
    <td class="whitespace-nowrap px-3 2xl:px-4 py-3 2xl:py-4 hidden xl:table-cell">
        @if($task->creator)
                ...
                <div class="relative h-6 w-6 md:h-8 md:w-8 flex-shrink-0">
                    @if($task->creator->avatar && $task->creator->avatar_url)
                        ...
                        <div
                            ...
                            <span class="text-xsfont-bold text-emerald-900 dark:text-emerald-200">
    ...
    <td class="whitespace-nowrap px-3 2xl:px-4 py-3 2xl:py-4 w-32">
        <div class="flex items-center gap-2">
            <!-- View Button -->
            <button @click="openPreview({{ $task->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-emerald-700/60 bg-emerald-500/10 p-1.5 transition-shadow duration-200 hover:border-emerald-700 hover:bg-emerald-500/20 hover:shadow-lg hover:shadow-emerald-700/30" title="{{ __('View Task') }}">
                <svg class="h-3.5 w-3.5 md:h-4 md:w-4 text-emerald-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <!-- Edit Button -->
            <button @click="openModal({{ $task->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-cyan-700/60 bg-cyan-500/10 p-1.5 transition-shadow duration-200 hover:border-cyan-700 hover:bg-cyan-500/20 hover:shadow-lg hover:shadow-cyan-700/30" title="{{ __('Edit Task') }}">
                <svg class="h-3.5 w-3.5 md:h-4 md:w-4 text-cyan-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            @endphp
            @if($canDeleteTask)
                <button @click="window.confirmDelete({{ $task->id }}, '{{ addslashes($task->title) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-task', { taskId: {{ $task->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-1.5 transition-shadow duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30" title="{{ __('Delete Task') }}">
                    <svg class="h-3.5 w-3.5 md:h-4 md:w-4 text-red-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
