...
<div class="group relative overflow-hidden {{ $rounded ? 'rounded-xl' : '' }} border border-violet-200 bg-white/50 p-4 backdrop-blur-sm transition-all duration-200 hover:border-violet-300 hover:shadow-lg dark:border-violet-900/50 dark:bg-gray-800/50 dark:hover:border-violet-800">
    <div class="flex flex-col gap-4">
        <!-- Header: Main info + Actions -->
        <div class="flex items-start justify-between gap-3">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-400/20 shadow-lg ring-2 ring-violet-300 dark:ring-violet-800 flex-shrink-0">
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white break-words whitespace-nowrap truncate">{{ $vendor->name }}</h3>
                    </div>
                </div>
            </div>
            ...
            @if($vendor->email)
                <div class="flex items-start gap-2">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $vendor->email }}</p>
                    </div>
                </div>
            @endif
            ...
            @if($vendor->phone)
                <div class="flex items-start gap-2">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $vendor->phone }}</p>
                    </div>
                </div>
            @endif
            ...
            @if($vendor->address)
                <div class="flex items-start gap-2">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $vendor->address }}</p>
                    </div>
                </div>
            @endif
        ...
        <div class="flex items-center justify-end border-t border-gray-200/50 pt-3 dark:border-gray-700/50">
            <div class="flex items-center gap-1.5">
                ...
                <!-- View Button -->
                <button @click="openPreview({{ $vendor->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-violet-700/60 bg-violet-500/10 p-2 transition-all duration-200 hover:border-violet-700 hover:bg-violet-500/20 hover:shadow-lg hover:shadow-violet-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('View Vendor') }}">
                ...
                <!-- Edit Button -->
                <button @click="openModal({{ $vendor->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-purple-700/60 bg-purple-500/10 p-2 transition-all duration-200 hover:border-purple-700 hover:bg-purple-500/20 hover:shadow-lg hover:shadow-purple-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Edit Vendor') }}">
                ...
                @if($canDelete)
                    @php
                        // Check for child records that will be affected
                    ...
                    @endphp
                    <button @click="window.confirmDelete({{ $vendor->id }}, '{{ addslashes($vendor->name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-vendor', { vendorId: {{ $vendor->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-2 transition-all duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Delete Vendor') }}">
