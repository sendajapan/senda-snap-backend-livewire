    ...
    <div x-data="{ open: @entangle('open') }"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="open"
                ...
                @if($user)
                    <div class="absolute bottom-0 left-0 right-0 flex items-center justify-between gap-3 border-t border-blue-200/30 bg-white/60 px-4 py-3 backdrop-blur-md dark:border-blue-800/30 dark:bg-gray-900/60">
                        <!-- Close Button (Left) -->
                        ...
                        <div class="flex items-center gap-2">
                            <!-- Edit Button (cyan) -->
                            <button wire:click="editUser" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-cyan-500/70 bg-cyan-600/20 backdrop-blur-sm px-3 py-2 transition-all duration-200 hover:border-cyan-400 hover:bg-cyan-500/30 hover:shadow-lg hover:shadow-cyan-500/50">
                            ...
                            @if($this->canDelete())
                                ...
                                @endphp
                                <button @click="window.confirmDelete({{ $user->id }}, '{{ addslashes($user->name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.deleteUser() } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-500/70 bg-red-600/20 backdrop-blur-sm px-3 py-2 transition-all duration-200 hover:border-red-400 hover:bg-red-500/30 hover:shadow-lg hover:shadow-red-500/50">
