...
<tr class="group transition-all duration-200 hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-teal-50/50 dark:hover:from-blue-900/10 dark:hover:to-teal-900/10">
    ...
    <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
        <div class="flex justify-center items-center gap-1.5 md:gap-2">
            ...
            <!-- Delete Button -->
            @if($canDelete)
                <button @click="window.confirmDelete({{ $port->id }}, '{{ addslashes($port->port_name) }}').then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-port', { portId: {{ $port->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-1.5 transition-all duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30" title="{{ __('Delete Port') }}">
