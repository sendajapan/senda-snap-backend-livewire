<div class="flex h-full w-full flex-1 flex-col gap-4" x-data="{
    ...
    <x-page-header
        ...
        variant="indigo">
        <x-slot:icon>
            <flux:icon.cube class="h-7 w-7 text-white" />
        </x-slot:icon>
        <x-slot:actions>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200/50 dark:divide-gray-700/50">
                    @forelse($shippingCompanies as $index => $shippingCompany)
                        <x-shipping-company-table-row :shippingCompany="$shippingCompany" :index="$shippingCompanies->firstItem() + $index" wire:key="shipping-company-row-{{ $shippingCompany->id }}" />
                    @empty
                        <tr>
