    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center justify-end border-t border-gray-200/50 pt-3 dark:border-gray-700/50">
            <div class="flex items-center gap-1">
                ...
                <!-- View Button -->
                <button @click="openPreview({{ $port->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-blue-700/60 bg-blue-500/10 p-2 transition-shadow duration-200 hover:border-blue-700 hover:bg-blue-500/20 hover:shadow-sm opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('View Port') }}">
                ...
                <!-- Edit Button -->
                <button @click="openModal({{ $port->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-cyan-700/60 bg-cyan-500/10 p-2 transition-shadow duration-200 hover:border-cyan-700 hover:bg-cyan-500/20 hover:shadow-sm opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Edit Port') }}">
                ...
                @if($canDelete)
                    ...
                    @endphp
                    <button @click="window.confirmDelete({{ $port->id }}, '{{ addslashes($port->port_name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-port', { portId: {{ $port->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-2 transition-shadow duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-sm opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Delete Port') }}">
