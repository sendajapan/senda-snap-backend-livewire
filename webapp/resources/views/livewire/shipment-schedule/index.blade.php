<div class="flex h-full w-full flex-1 flex-col gap-4" x-data="{
    ...
    <!-- Header Section -->
    <x-page-header :title="__('Shipment Schedule')" :description="__('Manage and view shipment schedules')"
        variant="blue">
        <x-slot:icon>
        ...
        <div class="mb-4 flex flex-wrap gap-4">
            <div class="flex-1 min-w-64">
                <flux:input wire:model.live.debounce.300ms="search"
                    placeholder="{{ __('Search by vessel name or voyage number...') }}" icon="magnifying-glass" />
            </div>
            <div class="w-48">
                <flux:input wire:model.live.debounce.300ms="vesselFilter"
                    placeholder="{{ __('Filter by vessel...') }}" />
            </div>
            <div class="w-48">
                <flux:input wire:model.live.debounce.300ms="voyageFilter"
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead>
                    <tr class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900">
                        ...
                        <th
                            class="px-3 md:px-6 py-3 md:py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300 w-12">
                        </th>
                        <th
                ...
                <tbody class="divide-y divide-gray-200/50 dark:divide-gray-700/50">
                    @forelse($schedules as $index => $schedule)
                        <tr wire:key="schedule-row-{{ $schedule->id }}"
                            class="group transition-all duration-200 hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-teal-50/50 dark:hover:from-blue-900/10 dark:hover:to-teal-900/10">
                            <!-- S/N -->
                            <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5 text-center">
                                <span
                                    class="text-xs md:text-sm font-semibold text-gray-600 dark:text-gray-400">{{ $schedules->firstItem() + $index }}</span>
                            </td>
                            <!-- Expand/Collapse Button -->
                            <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
                                <button @click="toggleSchedule({{ $schedule->id }})" type="button"
                                    class="flex items-center justify-center rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-gray-800 dark:hover:text-gray-300">
                                    <svg class="h-5 w-5 transition-transform duration-200"
                                        :class="{ 'rotate-90': isExpanded({{ $schedule->id }}) }" fill="none"
                            ...
                            <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
                                <div class="flex items-center">
                                    <div
                                        class="flex h-10 md:h-12 w-10 md:w-12 items-center justify-center rounded-xl bg-blue-400/20 shadow-lg ring-2 ring-blue-300 dark:ring-blue-800 flex-shrink-0">
                                        <svg class="h-5 md:h-6 w-5 md:w-6 text-blue-600 dark:text-blue-400" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <td class="px-3 md:px-6 py-3 md:py-5">
                                <div class="flex flex-wrap gap-1.5">
                                    @if($schedule->carrier1)
                                        <flux:badge color="blue" size="sm" class="text-xs whitespace-normal break-words">
                                            {{ $schedule->carrier1->line_name }}</flux:badge>
                                    @endif
                                    @if($schedule->carrier2)
                                        <flux:badge color="cyan" size="sm" class="text-xs whitespace-normal break-words">
                                            {{ $schedule->carrier2->line_name }}</flux:badge>
                                    @endif
                                    @if($schedule->carrier3)
                                        <flux:badge color="teal" size="sm" class="text-xs whitespace-normal break-words">
                                            {{ $schedule->carrier3->line_name }}</flux:badge>
                                    @endif
                            ...
                            <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
                                <div class="flex items-center gap-2">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs md:text-sm font-semibold text-gray-900 dark:text-white">{{ $schedule->startPort->port_name ?? 'N/A' }}</span>
                                    ...
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs md:text-sm font-semibold text-gray-900 dark:text-white">{{ $schedule->endPort->port_name ?? 'N/A' }}</span>
                                        <span
                                            class="text-xs text-gray-500 dark:text-gray-400">{{ __('Arrival Port') }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span
                                        class="text-xs md:text-sm text-gray-900 dark:text-white">{{ $schedule->eta ? $schedule->eta->format('M d, Y') : 'N/A' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5 w-32">
                                <div class="flex justify-center items-center gap-1.5 md:gap-2">
                                    ...
                                    <!-- Edit Button -->
                                    <button @click="openScheduleModal({{ $schedule->id }})" type="button"
                                        class="group relative flex items-center justify-center rounded-lg border-2 border-cyan-700/60 bg-cyan-500/10 p-1.5 transition-all duration-200 hover:border-cyan-700 hover:bg-cyan-500/20 hover:shadow-lg hover:shadow-cyan-700/30"
                                        title="{{ __('Edit Schedule') }}">
                                        <svg class="h-3.5 w-3.5 md:h-4 md:w-4 text-cyan-700 transition-all duration-200 group-hover:text-cyan-600"
                                    ...
                                    @if($canDelete)
                                        ...
                                        @endphp
                                        <button
                                            @click="confirmDeleteSchedule({{ $schedule->id }}, '{{ addslashes($schedule->vessel_name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-schedule', { scheduleId: {{ $schedule->id }} }) } })"
                                            type="button"
                                            ...
                                            <svg class="h-3.5 w-3.5 md:h-4 md:w-4 text-red-700 transition-all duration-200 group-hover:text-red-600"
                                                fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <tr wire:key="schedule-expand-{{ $schedule->id }}" x-show="isExpanded({{ $schedule->id }})"
                            x-collapse class="bg-gray-50/50 dark:bg-gray-800/30">
                            <td colspan="7" class="px-3 md:px-6 py-4">
                                <div
                                    class="ml-8 md:ml-12 border-l-2 border-dashed border-amber-400 dark:border-amber-600 pl-4 md:pl-6">
                                    <div class="flex items-center justify-between mb-3">
                                        ...
                                        @if($schedule->stopovers->count() === 0)
                                            <flux:button @click="openStopoverModal(null, {{ $schedule->id }})" size="sm"
                                                variant="outline" icon="plus">
                                                {{ __('Add Stopover') }}
                                            </flux:button>
                                        @endif
                                    </div>
                                    ...
                                    <div
                                        class="overflow-x-auto rounded-lg border-2 border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/20 p-3 shadow-md shadow-amber-200/50 dark:shadow-amber-900/30">
                                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            <thead>
                                                <tr class="bg-amber-200/70 dark:bg-amber-800/50">
                                                    <th
                                                        class="px-3 py-2 text-center text-xs font-semibold uppercase text-gray-700 dark:text-gray-300 w-16">
                                                        {{ __('S/N') }}</th>
                                                    <th
                                                        class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-700 dark:text-gray-300">
                                                        {{ __('Port') }}</th>
                                                    <th
                                                        class="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-700 dark:text-gray-300">
                                                        {{ __('Arrival (ETA)') }}</th>
                                                    <th
                                                        ...
                                                        {{ __('Departure (ETD)') }}</th>
                                                    <th
                                                        class="px-3 py-2 text-center text-xs font-semibold uppercase text-gray-700 dark:text-gray-300 w-32">
                                                        {{ __('Actions') }}</th>
                                                </tr>
                                            </thead>
                                            <tbody
                                                class="divide-y divide-gray-200/50 dark:divide-gray-700/50 bg-white/50 dark:bg-gray-800/20">
                                                @forelse($schedule->stopovers as $stopoverIndex => $stopover)
                                                    <tr wire:key="stopover-{{ $stopover->id }}"
                                                        class="hover:bg-amber-100/70 dark:hover:bg-amber-800/40">
                                                        <td class="px-3 py-2 whitespace-nowrap text-center">
                                                            <span
                                                                class="text-xs font-semibold text-gray-600 dark:text-gray-400">{{ $stopoverIndex + 1 }}</span>
                                                        </td>
                                                        <td class="px-3 py-2 whitespace-nowrap">
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <span
                                                                    class="text-xs md:text-sm text-gray-900 dark:text-white">{{ $stopover->port->port_name ?? 'N/A' }}</span>
                                                            </div>
                                                        </td>
                                                        <td class="px-3 py-2 whitespace-nowrap">
                                                            @if($stopover->stopover_eta)
                                                                <div class="flex items-center gap-1.5">
                                                                    ...
                                                                    <span
                                                                        class="text-xs md:text-sm text-gray-900 dark:text-white">{{ $stopover->stopover_eta->format('M d, Y') }}</span>
                                                                </div>
                                                            @else
                                                                <span
                                                                    class="text-xs text-gray-400 dark:text-gray-500">{{ __('Not set') }}</span>
                                                            @endif
                                                        </td>
                                                        <td class="px-3 py-2 whitespace-nowrap">
                                                            @if($stopover->stopover_etd)
                                                                <div class="flex items-center gap-1.5">
                                                                    <svg class="h-4 w-4 text-orange-500" fill="none"
                                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                                            stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                                                                    </svg>
                                                                    <span
                                                                        class="text-xs md:text-sm text-gray-900 dark:text-white">{{ $stopover->stopover_etd->format('M d, Y') }}</span>
                                                                </div>
                                                            @else
                                                                <span
                                                                    class="text-xs text-gray-400 dark:text-gray-500">{{ __('Not set') }}</span>
                                                        ...
                                                        <td class="px-3 py-2 whitespace-nowrap w-32">
                                                            <div class="flex justify-center items-center gap-1.5">
                                                                <button
                                                                    @click="openStopoverModal({{ $stopover->id }}, {{ $schedule->id }})"
                                                                    type="button"
                                                                    class="rounded-lg border-2 border-cyan-700/60 bg-cyan-500/10 p-1.5 text-cyan-700 transition-all hover:bg-cyan-500/20"
                                                                ...
                                                                @if($canDelete)
                                                                    <button
                                                                        @click="confirmDeleteStopover({{ $stopover->id }}, '{{ addslashes($stopover->port->port_name ?? 'N/A') }}').then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-stopover', { stopoverId: {{ $stopover->id }} }) } })"
                                                                        type="button"
                                                                        class="rounded-lg border-2 border-red-700/60 bg-red-500/10 p-1.5 text-red-700 transition-all hover:bg-red-500/20"
                                                                        title="{{ __('Delete Stopover') }}">
                                                                        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor"
                                                                            viewBox="0 0 24 24">
                                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                                stroke-width="2"
                                                        ...
                                                        <td colspan="5" class="px-3 py-6 text-center">
                                                            <div class="flex flex-col items-center gap-3">
                                                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                                                    {{ __('No stopovers added yet') }}</p>
                                                                <flux:button
                                                                    @click="openStopoverModal(null, {{ $schedule->id }})"
        ...
        <div class="2xl:hidden bg-white/50 backdrop-blur-sm dark:bg-gray-900/20"
            ...
            <div class="grid grid-cols-1 gap-4 p-4">
                @forelse($schedules as $schedule)
                    <div
                        class="rounded-xl border border-blue-200 bg-white/50 p-4 shadow-md shadow-blue-100/50 dark:border-blue-900/50 dark:bg-gray-800/50 dark:shadow-blue-900/20">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="text-sm font-bold text-gray-900 dark:text-white">{{ $schedule->vessel_name }}
                                    </h3>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('Voyage:') }}
                                        {{ $schedule->voyage_no }}</p>
                                </div>
                            </div>
                            <div class="flex gap-1">
                                <button @click="openScheduleModal({{ $schedule->id }})"
                                    class="rounded-lg p-1.5 text-cyan-700 hover:bg-cyan-500/10">
                        ...
                        <div class="space-y-2 text-xs">
                            <div>
                                ...
                                <span class="text-gray-900 dark:text-white">{{ $schedule->startPort->port_name ?? 'N/A' }} →
                                    {{ $schedule->endPort->port_name ?? 'N/A' }}</span>
                            </div>
                            <div>
                                <span class="font-semibold text-gray-700 dark:text-gray-300">{{ __('ETA:') }}</span>
                                <span
                                    class="text-gray-900 dark:text-white">{{ $schedule->eta ? $schedule->eta->format('M d, Y') : 'N/A' }}</span>
                            </div>
                            @if($schedule->stopovers->count() > 0)
                                <div>
