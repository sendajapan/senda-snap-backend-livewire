        ...
        <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                @if($canDelete)
                    ...
                    @endphp
                    <button @click="window.confirmDelete({{ $user->id }}, '{{ addslashes($user->name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-user', { userId: {{ $user->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-2 transition-all duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30" title="{{ __('Delete User') }}">
