...
<div class="group relative w-full overflow-hidden rounded-lg border {{ $color['border'] }} {{ $color['bg'] }} p-3 shadow-md transition-shadow duration-200 {{ $color['hoverBorder'] }} hover:shadow-lg backdrop-blur-sm">
    <div class="flex flex-col gap-2">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-1 flex-wrap">
                    ...
                    @if($task->attachments && $task->attachments->count() > 0)
                        <div class="flex items-center gap-1 rounded {{ $color['attachmentBg'] }} px-1 py-0.5 flex-shrink-0" title="{{ $task->attachments->count() }} {{ __('attachment(s)') }}">
                            ...
                            <span class="text-xs font-semibold {{ $color['attachmentText'] }}">{{ $task->attachments->count() }}</span>
                        </div>
                    @endif
                </div>
                @if($task->description)
            ...
            <div class="flex items-center gap-1">
                <div class="flex -space-x-1.5">
                        ...
                        <div class="relative h-5 w-5 flex-shrink-0" title="{{ $user->name }}">
                            ...
                            @else
                                <div class="flex h-5 w-5 items-center justify-center rounded-lg {{ $color['avatarBg'] }} ring-2 ring-white dark:ring-gray-900">
                                    <span class="text-xs font-bold {{ $color['avatarText'] }}">{{ $user->initials() }}</span>
                                </div>
                            @endif
                        </div>
                    @endforeach
                    @if($task->assignedUsers->count() > 2)
                        <div class="flex h-5 w-5 items-center justify-center rounded-lg {{ $color['avatarPlus'] }} ring-2 ring-white dark:ring-gray-900">
                            <span class="text-xs font-bold text-white">+{{ $task->assignedUsers->count() - 2 }}</span>
                        </div>
                    @endif
                </div>
                <span class="truncate text-xs text-gray-600 dark:text-gray-400">
                    ...
                    @if($task->assignedUsers->count() > 1)
                        <span class="text-xs">+{{ $task->assignedUsers->count() - 1 }}</span>
                    @endif
                </span>
            </div>
        @endif
        ...
        <div class="flex justify-end mt-2">
            <button @click.stop="openPreview({{ $task->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-emerald-700/60 bg-emerald-500/10 px-2 py-1.5 transition-all duration-200 hover:border-emerald-700 hover:bg-emerald-500/20 hover:shadow-sm" title="{{ __('Preview Task') }}">
