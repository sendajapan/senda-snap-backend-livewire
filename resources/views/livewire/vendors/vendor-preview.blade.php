            ...
            <div x-show="open"
                ...
                <div class="absolute bottom-0 left-0 right-0 grid grid-cols-3 items-center gap-3 border-t border-violet-200/30 bg-white/60 px-4 py-3 backdrop-blur-md dark:border-violet-800/30 dark:bg-gray-900/60">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        ...
                        @if($this->canDelete())
                            <button @click="window.confirmDelete({{ $vendor->id }}, '{{ addslashes($vendor->name) }}').then((result) => { if (result.isConfirmed) { $wire.deleteVendor() } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-500/70 bg-red-600/20 backdrop-blur-sm px-3 py-2 transition-all duration-200 hover:border-red-400 hover:bg-red-500/30 hover:shadow-lg hover:shadow-red-500/50">
