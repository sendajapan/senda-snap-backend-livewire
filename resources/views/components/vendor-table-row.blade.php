...
<tr class="group transition-all duration-200 hover:bg-gradient-to-r hover:from-violet-50/50 hover:to-purple-50/50 dark:hover:from-violet-900/10 dark:hover:to-purple-900/10">
    ...
    <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
        <div class="flex items-center gap-1.5 md:gap-2">
            ...
            <span class="text-xs md:text-sm text-gray-900 dark:text-white truncate max-w-[200px] md:max-w-none">{{ $vendor->email }}</span>
        </div>
    </td>
    <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
        @if($vendor->phone)
            <div class="flex items-center gap-1.5 md:gap-2">
                ...
                <span class="text-xs md:text-sm text-gray-900 dark:text-white">{{ $vendor->phone }}</span>
            </div>
        @else
            <span class="text-xs md:text-sm text-gray-400 dark:text-gray-500">{{ __('N/A') }}</span>
        @endif
    </td>
    <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5">
    ...
    <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5 hidden lg:table-cell">
        <div class="flex flex-col">
            <span class="text-xs md:text-sm text-gray-900 dark:text-white">{{ $vendor->created_at->format('M d, Y') }}</span>
    ...
    <td class="whitespace-nowrap px-3 md:px-6 py-3 md:py-5 w-32">
        <div class="flex justify-center items-center gap-1.5 md:gap-2">
            ...
            @if($canDelete)
                ...
                @endphp
                <button @click="window.confirmDelete({{ $vendor->id }}, '{{ addslashes($vendor->name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-vendor', { vendorId: {{ $vendor->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-1.5 transition-all duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30" title="{{ __('Delete Vendor') }}">
