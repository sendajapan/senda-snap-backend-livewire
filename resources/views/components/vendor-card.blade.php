    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="flex items-center justify-end border-t border-gray-200/50 pt-3 dark:border-gray-700/50">
            <div class="flex items-center gap-1.5">
                ...
                <!-- Delete Button -->
                @if($canDelete)
                    <button @click="window.confirmDelete({{ $vendor->id }}, '{{ addslashes($vendor->name) }}').then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-vendor', { vendorId: {{ $vendor->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-2 transition-all duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Delete Vendor') }}">
