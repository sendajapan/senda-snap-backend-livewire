    ...
    <div class="flex flex-col gap-4">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white break-words whitespace-nowrap truncate">{{ $port->port_name }}</h3>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 gap-3">
            ...
            <div class="flex items-start gap-2">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ $port->port_address }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-gray-900 dark:text-white whitespace-nowrap">{{ $port->created_at->format('M d, Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end border-t border-gray-200/50 pt-3 dark:border-gray-700/50">
            <div class="flex items-center gap-1.5">
                ...
                <!-- View Button -->
                <button @click="openPreview({{ $port->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-blue-700/60 bg-blue-500/10 p-2 transition-all duration-200 hover:border-blue-700 hover:bg-blue-500/20 hover:shadow-lg hover:shadow-blue-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('View Port') }}">
                ...
                <!-- Edit Button -->
                <button @click="openModal({{ $port->id }})" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-cyan-700/60 bg-cyan-500/10 p-2 transition-all duration-200 hover:border-cyan-700 hover:bg-cyan-500/20 hover:shadow-lg hover:shadow-cyan-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Edit Port') }}">
                ...
                @if($canDelete)
                    ...
                    @endphp
                    <button @click="window.confirmDelete({{ $port->id }}, '{{ addslashes($port->port_name) }}', @js($warnings)).then((result) => { if (result.isConfirmed) { $wire.$dispatch('delete-port', { portId: {{ $port->id }} }) } })" type="button" class="group relative flex items-center justify-center rounded-lg border-2 border-red-700/60 bg-red-500/10 p-2 transition-all duration-200 hover:border-red-700 hover:bg-red-500/20 hover:shadow-lg hover:shadow-red-700/30 opacity-50 transition-opacity group-hover:opacity-100" title="{{ __('Delete Port') }}">
